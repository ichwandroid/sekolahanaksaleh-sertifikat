// ============================================
// Sertifikat Assembly SD Anak Saleh
...
certificateForm.addEventListener('submit', function (e) {
    ...
    // Find selected student data to get the level
    const selectedStudent = dataSiswa.find(siswa => siswa.nama === nama && siswa.kelas === kelas);
    const level = selectedStudent ? selectedStudent.level : '1'; // Default to '1' if not found
...
async function downloadCertificate(nama, kelas, level) {
    ...
    try {
        ...
        template.src = 'assets/template_assembly.png';
        ...
        doc.text(nama, width / 2, 95, { align: "center" });

        // 3. Class
        doc.setFont("helvetica", "normal");
        doc.setFontSize(20);
        doc.setTextColor(55, 65, 81); // Gray-700
        doc.text(`Kelas ${kelas}`, width / 2, 110, { align: "center" });

        // 3. Description
        doc.setFont("helvetica", "normal");
        doc.setFontSize(14);
        doc.setTextColor(75, 85, 99); // Gray-600
        const text = "Atas partisipasinya yang luar biasa dalam kegiatan Assembly";
        doc.text(text, width / 2, 120, { align: "center" });
        doc.text("SD Anak Saleh Malang Tahun Ajaran 2025/2026. ", width / 2, 125, { align: "center" });
        doc.text("Kreativitas, keberanian dan kerja sama yang kamu tunjukkan ", width / 2, 130, { align: "center" });
        doc.text("menjadi inspirasi bagi teman-teman lainnya.Teruslah berkarya dan menjadi pribadi yang unggul!", width / 2, 135, { align: "center" });
        ...
        doc.setTextColor(75, 85, 99); // Gray-600
        if (level === '1') {
            doc.text("Malang, 10 Desember 2025", width / 2, 150, { align: "center" });
        } else if (level === '2') {
            doc.text("Malang, 09 Desember 2025", width / 2, 150, { align: "center" });
        } else if (level === '3') {
            doc.text("Malang, 11 Desember 2025", width / 2, 150, { align: "center" });
        } else if (level === '4') {
            doc.text("Malang, 11 Desember 2025", width / 2, 150, { align: "center" });
        } else if (level === '5') {
            doc.text("Malang, 08 Desember 2025", width / 2, 150, { align: "center" });
        } else if (level === '6') {
            doc.text("Malang, 08 Desember 2025", width / 2, 150, { align: "center" });
        }
...
function setLoadingState(isLoading) {
    if (isLoading) {
        ...
        btnText.textContent = 'Download Sertifikat';
...
function showToast(message, type = 'success') {
    ...
    const toastContent = toastContainer.querySelector('div');
